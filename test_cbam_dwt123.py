...
from utils import *
from networks.generator_cbam123 import BRN, print_network
import time

parser = argparse.ArgumentParser(description="AID_DWT_Test")
parser.add_argument("--logdir", type=str, default='./logs/DWT/Rain200H_cbam_100/')
parser.add_argument("--data_path", type=str, default='/media/ubuntu/Seagate/RainData/Rain200H/test/small/rain/')
parser.add_argument("--save_path", type=str, default='./results/DWT/Rain200H_CBAM_100/')
parser.add_argument('--save_path_r', type=str, default='./results/DWT/Rain200H_CBAM_100/streak/')
...
def main():
    ...
    if opt.use_GPU:
        model = model.cuda()
    state_dict = torch.load(os.path.join(opt.logdir, 'net_epoch95.pth'))
