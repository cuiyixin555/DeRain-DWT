...
parser.add_argument("--save_path", type=str, default='./logs/DWT/Rain1200_cbam_100_new/')
parser.add_argument("--save_freq", type=int, default=1, help='save intermediate model')
parser.add_argument("--data_path", type=str, default='/media/ubuntu/Seagate/RainData/Rain1200/train/small/')
...
def main():
    ...
    dataset_train = TrainValDataset(data_path=opt.data_path, patch_size=100)
    ...
    scheduler = MultiStepLR(optimizer, milestones=[60, 80], gamma=0.2)  # learning rates
    scheduler_D = MultiStepLR(optimizer, milestones=[60, 80], gamma=0.2)
    ...
    for epoch in range(initial_epoch, opt.epochs):
        ...
        # train
        for i, sample in enumerate(loader_train, 0):
            ...
            # read original data
            input_train = Variable(sample['O'].cuda())
            target_train = Variable(sample['B'].cuda())
            clear_train = Variable(sample['C'].cuda())
