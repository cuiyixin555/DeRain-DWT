...
class RGA_Module(nn.Module):
    def __init__(self, in_channel, in_spatial, use_spatial=True, use_channel=True, cha_ratio=8, spa_ratio=8, down_ratio=8):
        ...
        self.inter_channel = in_channel // cha_ratio # 4
        self.inter_spatial = in_spatial // spa_ratio # 4
        ...
        if self.use_spatial:
            self.gg_spatial = nn.Sequential(
                nn.Conv2d(in_channels=self.in_spatial * 2, out_channels=self.inter_spatial, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.inter_spatial),
                nn.ReLU()
            )
        ...
        if self.use_spatial:
            ...
            self.W_spatial = nn.Sequential(
                nn.Conv2d(in_channels=num_channel_s, out_channels=num_channel_s // down_ratio, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(num_channel_s // down_ratio),
                nn.ReLU(),
                nn.Conv2d(in_channels=num_channel_s // down_ratio, out_channels=1, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(1)
            )
    ...
    def forward(self, x):
        ...
        if self.use_spatial:
            ...
            print('Gs_in: ', Gs_in.shape)
            Gs_out = Gs.view(b, h * w, h, w)
            print('Gs_out: ', Gs_out.shape)
            Gs_joint = torch.cat((Gs_in, Gs_out), 1)
            print('Gs_joint: ', Gs_joint.shape)
